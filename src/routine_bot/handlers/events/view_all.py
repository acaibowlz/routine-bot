...
from routine_bot.utils import format_logger_name, get_time_diff
...
def handle_view_all_chat(user_id: str, conn: psycopg.Connection) -> FlexMessage:
    ...
    logger.info(f"Retrieved {len(user_events)} events for user_id: {user_id}")
    shared_events = share_db.list_shared_events_by_user(user_id, conn)
    logger.info(f"Retrieved {len(shared_events)} shared events for user_id: {user_id}")
