from linebot.v3.messaging import ButtonsTemplate, FlexMessage, MessageAction, TemplateMessage

from routine_bot.enums.options import ConfirmDeletionOptions
from routine_bot.messages.utils import flex_bubble_template


def enter_event_name() -> FlexMessage:
    bubble = flex_bubble_template(title="🍞 刪除事項", lines=["📝 請輸入要刪除的事項名稱"])
    return FlexMessage(altText="🍞 請輸入要刪除的事項名稱", contents=bubble)


def comfirm_event_deletion(chat_payload: dict[str, str]) -> TemplateMessage:
    if chat_payload["reminder_enabled"] == "True":
        text = (
            f"\n🗓 上次是：{chat_payload['last_done_at']}\n\n"
            f"🔔 下次提醒：{chat_payload['next_due_at']}\n\n"
            "✨ 確定要刪除這個事項嗎？"
        )
    else:
        text = f"\n🗓 上次是：{chat_payload['last_done_at']}\n\n🔕 提醒設定：關閉\n\n✨ 確定要刪除這個事項嗎？"
    template = ButtonsTemplate(
        title=f"🍞 刪除［{chat_payload['event_name']}］",
        text=text,
        actions=[
            MessageAction(label="是", text=ConfirmDeletionOptions.DELETE.value),
            MessageAction(label="否", text=ConfirmDeletionOptions.CANCEL.value),
        ],
    )
    msg = TemplateMessage(altText=f"🍞 確定要刪除［{chat_payload['event_name']}］嗎？", template=template)
    return msg


def succeeded(chat_payload: dict[str, str]) -> FlexMessage:
    bubble = flex_bubble_template(title=f"🍞 刪除［{chat_payload['event_name']}］", lines=["✅ 已成功刪除！"])
    return FlexMessage(altText=f"✅［{chat_payload['event_name']}］已成功刪除！", contents=bubble)


def cancelled(chat_payload: dict[str, str]) -> FlexMessage:
    bubble = flex_bubble_template(title=f"🍞 刪除［{chat_payload['event_name']}］", lines=["🚫 已取消刪除"])
    return FlexMessage(altText="🚫 已取消刪除", contents=bubble)


def invalid_delete_confirmation(chat_payload: dict[str, str]) -> TemplateMessage:
    text = "\n⚠️ 嗯？我不太確定你的意思\n\n✨ 請幫我從下方按鈕確認是否刪除"
    template = ButtonsTemplate(
        title=f"🍞 刪除［{chat_payload['event_name']}］",
        text=text,
        actions=[
            MessageAction(label="是", text="刪除事項"),
            MessageAction(label="否", text="取消刪除"),
        ],
    )
    msg = TemplateMessage(altText=f"⚠️ 輸入無效，確定要刪除［{chat_payload['event_name']}］嗎？", template=template)
    return msg
